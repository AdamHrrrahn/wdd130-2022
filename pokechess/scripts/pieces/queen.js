function Queen(pos, dir, id, board){
    this.type = 'queen'
    this.board = board
    this.pieces = null
    this.id = id
    ...
    this.get_moves = function(){
        ...
        for (let i = 1; (a+i < 9); i++){
            this.move_list.push((a + i) + '_' + (b))
            if (!(this.board[a + i][b] === null)){
                if (this.check_friendly(a+i,b)){
                    this.protected.push(this.move_list.pop())
                } else if (this.board[a+i][b].type == 'king'){
                    this.protected.push((a+i+1) + '_' + b)
                }
                break;
            }
        }
        ...
        for (let i = 1; (i+b < 9); i++){
            this.move_list.push((a) + '_' + (b + i))
            if (!(this.board[a][b + i] === null)){
                if (this.check_friendly(a,b+i)){
                    this.protected.push(this.move_list.pop())
                } else if (this.board[a][b+i].type == 'king'){
                    this.protected.push(a + '_' + (b+i+1))
                }
                break;
            }
        }
